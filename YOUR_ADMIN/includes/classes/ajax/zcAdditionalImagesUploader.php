...
//        $addtl_image->set_filename($new_filename);
  ...
  function searchProducts() {
    ...
    foreach ($products_query_raw as $product) {
      $category = '';
      $cpath = zen_get_product_path((int)$product['products_id']);
  ...
  function checkProductID() {
    ...
    $products_query = $db->Execute("SELECT p.products_id, pd.products_name, p.products_image
                                    FROM " . TABLE_PRODUCTS . " p,
                                         " . TABLE_PRODUCTS_DESCRIPTION . " pd
                                    WHERE p.products_id = pd.products_id
                                    AND pd.language_id = " . (int)$_SESSION['languages_id'] . "
